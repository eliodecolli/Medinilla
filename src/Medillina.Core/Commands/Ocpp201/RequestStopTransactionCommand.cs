...
internal sealed class RequestStopTransactionCommand(ILogger<RequestStopTransactionCommand> log) : IOcppChargerCommand
{
    public string Action => OcppActionNames.RequestStopTransaction;

    public async Task HandleError(string clientIdentifier, OcppCallError error, ICommandExecutionService executionService)
    {
        log.LogError("(RequestStopTransaction) {mi}: Errored: {err}, Error Details: {errd}, Error Code: {errc}",
    ...
    {
        var response = result.As<RequestStopTransactionResponse>();
        if (response is null)
        {
        ...
        log.LogInformation("(RequestStopTransaction) {mi}: status={status}",
            result.MessageId, info);
