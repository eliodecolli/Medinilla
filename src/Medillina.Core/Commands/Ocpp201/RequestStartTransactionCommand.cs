...
internal sealed class RequestStartTransactionCommand(ILogger<RequestStartTransactionCommand> log) : IOcppChargerCommand
{
    public string Action => OcppActionNames.RequestStartTransaction;

    public async Task HandleError(string clientIdentifier, OcppCallError error, ICommandExecutionService executionService)
    {
        log.LogError("(RequestStartTransaction) {mi}: Errored: {err}, Error Details: {errd}, Error Code: {errc}",
    ...
    {
        var response = result.As<RequestStartTransactionResponse>();
        if (response is null)
        {
        ...
        var failed = response.Status != RequestStartStopStatusEnum.Accepted;
        ...
        log.LogInformation("(RequestStartTransaction) {mi}: status={status}, transactionId={txId}",
            result.MessageId, info, response.TransactionId ?? "None");
