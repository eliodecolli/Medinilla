...
internal sealed class GetBaseReportCommand(ILogger<GetBaseReportCommand> log) : IOcppChargerCommand
{
    public string Action => OcppActionNames.GetBaseReport;

    public async Task HandleError(string clientIdentifier, OcppCallError error, ICommandExecutionService executionService)
    {
        log.LogError("(GetBaseReport) {mi}: Errored: {err}, Error Details: {errd}, Error Code: {errc}",
    ...
    {
        var response = result.As<GetBaseReportResponse>();
        if (response is null)
        {
        ...
        var failed = response.Status != GenericDeviceModelStatusEnum.Accepted;
        ...
        log.LogInformation("(GetBaseReport) {mi}: status={status}", result.MessageId, info);
